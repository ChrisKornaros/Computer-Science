...
- Use `Cmd + Shift + i` to insert a code block
...
- Up until now, I've been testing code with print statements
  - It's common and often the best practice to write your own code to test your programs
- Consider the following code, called `calculator.py` in my current working directory
```{python}
#| eval: false
...
- You could manually test simple numbers such as 2, but for more complex outputs this isn't a scalable practice
- You can create a simple script to test your code as follows
  - **Notice:** I'm in the same working directory, so I can call the name of the .py file as I would a library, and then specify the function
```{python}
#| eval: false
...
- Running this code will output nothing, which could mean we passed our tests or it didn't encounter a corner case that throws an error
- If we want to test more cases, our code can easily expand beyond a manageable size

### assert {#sec-assert}
- Python's `assert` command lets us tell the compiler something (an assertion) is true
  - You could modify the previous test code to remove both if statements and `assert square(2) == 4` `assert square(3) == 9`
  - Definitively asserting what `square(2)` and `square(3)` equal reduces the lines of code
- You can then break your code by changing the `square()` function to equal `n + n`
  - Subsequently running `test_calculator()` will then return an `AssertionError`, which means a condition was not met
  - That being said, you could see how even this format will become bloated at scale if you specify issues with code

### pytest {#sec-pytest}
- `pytest` is a third-party library that allows you to unit test your program, aka you can test functions within a program
- In your command-line, typing `pytest test_calculator.py` will provide output/feedback on the functions within your script
- In the current format, `pytest` will test all assertions at once, but if one line fails, then it will stop testing the rest
  - Modifying the code, so it has various testing categories allows you to narrow down the issues with your code
```{python}
#| eval: false
...
- Testing frameworks like `pytest` will run each function, even if one fails
  - This framework in particular has useful functions built-in to enable more robust testing and error responses
  - Instead of using assert, below, the `raises` function from `pytest` allows you to express the expectation of an error
  - Learn more in the official [pytest]<https://docs.pytest.org/en/7.1.x/getting-started.html> documentation
```{python}
#| eval: false
...
::: {#tip-testing_strings .callout-tip} 
#### Testing Strings
If you want to test strings with `pytest`, make sure that your function `return`s a value, instead of only `print(str)`. `pytest` works by testing function output and requires the `return` clause.
:::

### Organizing Tests Into Folders {#sec-organizing_tests_into_folders}
- Unit testing code with multiple tests is so common you have the ability to run a folder of tests with one command
- For example, in your working directory run `mkdir test`
  - You can then run `code test/test.py`
  - Add a `.py` file that you want to test to the test folder
  - Additionally, add a `__init__.py` file to the test directory
    - `pytest` doesn't allow you to run tests as a folder without the special `__init__` file
    - Even leaving this file blank tells `pytest` that the whole folder has tests that can be run
    - Learn more in the `pytest` documentation on [Import Mechanisms]<https://docs.pytest.org/en/7.1.x/explanation/pythonpath.html?highlight=folder#pytest-import-mechanisms-and-sys-path-pythonpath>
...
### File I/O {#sec-file_i/o}
- Up until now, all the code written was stored in memory: once the program ends, all information gathered/stored is lost
- File I/O (input/output) lets you store data so you can access it later
  - Writing to flat files (.txt, .csv)
  - Writing to databases (PostgreSQL, DuckDB)
  - Etc.

### open {#sec-open}
- `open` is a built-in Python function that allows you to open a file and utilize its contents in a program
  - You can both read from it and write to it
- Here's a basic example of how to create a file and then write to it
  - The `w` in `open()` tells Python to write to the file
  - If you just wanted to read a file's contents, you can use `r`
  - If you want to modify a file, instead of overwriting it, use `a`
  - If you want to create a file, you can use `x`, which returns an error if the file exists
- Furthermore, you can specify how the file should be handled
  - `t` for Text, the default mode
  - `b` for Binary, (i.e. images)
...
- Say you wanted to read and print out all of the lines in a file, with is great for interacting with that file
- This code is quite simple
  - It opens the file from earlier, `names.txt`, and stores each line in a list object
  - Then, it prints out each line, stripped of any excess line breaks or white space on each side
```{python}
...
- `CSV` stands for ***comma separated values***
  - Like its name implies, this file format separates the values within it via commas
  - This is a common format for storing and sharing data
  - For a classic, relation DB the first line(s) of the file would be the header, each column separated by a column
  - Then, subsequent rows would have their values from left to right stored, in accordance with the column headers
- While a `.csv` file is straightforward enough, it can be complicated, luckily there's the `csv` package in Python
  - There's a handy tool in the library called `reader` which helps handle embedded commas, among other things
    - Think about fields that may have commas, or comment boxes where a comma may be within quotes
  - There's also the `DictReader` object, which enables even more flexible csv functionality
    - This will let you specify columns in your code based on a header line in the .csv file
    - Doing so ensures that your code will always run so long as the file is configured properly
      - This technique is called `coding defensively`
- As always, learn more by reading Python's official [CSV]<https://docs.python.org/3/library/csv.html> documentation

### Binary Files and `PIL` {#sec-binary_files_pil}
- A binary file is, as it sounds, simply a collection of 1s and 0s, this type of file can store anything
- `PIL` is a popular Python library that works well with image files
- Animated GIFs are files where numerous images are played in a looping sequence to create a simple animation effect
- You can create simplistic gifs using PIL
  - Take an image, then combine it with the reading/writing/appending techniques to output the .gif
